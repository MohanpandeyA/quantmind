...
_SEARCH_INDEX: list[dict[str, Any]] = [
    {
        ...
        "exchange": exch,
        "type": typ,
        "type_display": {"EQUITY": "Equity", "ETF": "ETF", "MUTUALFUND": "Fund"}.get(
            typ, typ
        ),
...
def _search_curated(query: str, limit: int = 8) -> list[dict[str, Any]]:
    ...
    return [{k: v for k, v in r.items() if k != "_search"} for r in results[:limit]]
...
async def search_ticker(
    ...
    try:
        async with httpx.AsyncClient(
            timeout=4.0, headers=HEADERS, follow_redirects=True
        ) as client:
        ...
        logger.warning(
            "Ticker search | yahoo failed | q=%r | %s — using curated list",
            q,
            type(e).__name__,
        )
